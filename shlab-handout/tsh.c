...
/* signal.h 中需要定义该macro才能启用 struct sigaction */
...
int builtin_cmd(char **argv) 
{
    if(!strcmp(argv[0], BI_QUIT))
        exit(0);
    if(!strcmp(argv[0], BI_JOBS)){
        listjobs(jobs);
        return 1;
    }
        
    if ( !strcmp(argv[0], BI_BG) || !strcmp(argv[0], BI_FG)){
        do_bgfg(argv);  
        return 1;     
    }
...
inline int is_number(const char* id){
    for(char *c = (char*) id;*c!='\0'; c++){
            if(*c < '0'|| *c > '9'){ 
                return 0;
            }
    }
